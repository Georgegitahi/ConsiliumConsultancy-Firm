...
const Footer = () => {
  ...
  const handleNewsletterSubmit = async () => {
    ...
    try {
      ...
      // Custom message for newsletter subscription
      const customMessage = `
NEWSLETTER SUBSCRIPTION

A new user has subscribed to the Consilium newsletter:

Email: ${email}
Subscription Date: ${new Date().toLocaleDateString()}
Subscription Time: ${new Date().toLocaleTimeString()}

---
This subscription was made through the Consilium website footer.
      `;
    ...
    <footer className="relative bg-gradient-to-br from-slate-900 via-gray-900 to-black text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-16">
        <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-12">
          ...
          {/* Enhanced Newsletter with Web3Forms */}
          <div className="transform transition-all duration-300 hover:translate-y-[-2px]">
            <h3 className="text-xl font-semibold mb-6 text-gray-100 relative">
              Stay Informed
            ...
            {submitStatus === 'success' && (
              <div className="mb-4 p-3 bg-green-900/50 border border-green-500/50 rounded-lg">
                ...
                <p className="text-green-300/70 text-xs mt-1">Thank you for joining our newsletter.</p>
              </div>
            )}
            
            {submitStatus === 'error' && (
              <div className="mb-4 p-3 bg-red-900/50 border border-red-500/50 rounded-lg">
                <p className="text-red-300 text-sm font-medium">❌ Subscription failed</p>
                <p className="text-red-300/70 text-xs mt-1">Please try again or contact support.</p>
              </div>
            )}
            
            {submitStatus === 'validation_error' && (
              <div className="mb-4 p-3 bg-yellow-900/50 border border-yellow-500/50 rounded-lg">
                <p className="text-yellow-300 text-sm font-medium">⚠️ Please enter a valid email address</p>
              </div>
            )}
            ...
            <div className="space-y-3">
              <div className="relative group">
                <input
                  ...
                  required
                  className="w-full p-4 pr-12 rounded-xl bg-gray-800/70 backdrop-blur-sm border border-gray-600 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 hover:bg-gray-800/90 focus:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <div className="absolute right-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-blue-400 transition-colors duration-300">
                  {isSubmitting ? (
                    <div className="w-4 h-4 border-2 border-gray-400 border-t-blue-400 rounded-full animate-spin"></div>
                  ) : (
                    <span>📧</span>
                  )}
                </div>
